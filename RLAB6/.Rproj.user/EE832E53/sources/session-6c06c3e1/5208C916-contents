...
#1
inf1 <- read.csv("Dish.csv", fileEncoding = "Windows-1251", header = TRUE, sep = ",", check.names = F)

inf1 <- subset(inf1, select = -description)
inf1$first_appeared[inf1$first_appeared==1] <- NA
inf1$first_appeared[inf1$first_appeared==0] <- NA
inf1$last_appeared[inf1$last_appeared==0] <- NA
inf1$lowest_price[inf1$lowest_price==0] <- NA
inf1$highest_price[inf1$highest_price==0] <- NA
inf1 <- na.omit(inf1)

hist(inf1$lowest_price,xlim=c(0, 30), breaks = 10000, main="lowest_price")
hist(inf1$highest_price,xlim=c(0, 60), breaks = 20000, main="highest_price")
hist(inf1$first_appeared,xlim=c(1800, 2023), breaks = 100, main="first_appeared")
hist(inf1$last_appeared,xlim=c(1800, 2023), breaks = 100, main="last_appeared")

boxplot( inf1$lowest_price[inf1$lowest_price < mean(inf1$lowest_price) + 3*sd(inf1$lowest_price)], ylim=c(min(inf1$lowest_price),max(4)) ,
         boxwex = 0.5,
         medcol = "red")
boxplot( inf1$highest_price[inf1$highest_price < mean(inf1$highest_price) + 3*sd(inf1$highest_price)], ylim=c(min(inf1$highest_price),max(4)) ,
         boxwex = 0.5,
         medcol = "red")
...
inf2 <- inf1
inf2$lowest_price[inf2$lowest_price > 100] <- NA
inf2$highest_price[inf2$highest_price > 200] <- NA
inf2$first_appeared[inf2$first_appeared > 2023] <- NA
inf2$last_appeared[inf2$last_appeared > 2023] <- NA
inf2 <- na.omit(inf2)

ggplot(data = inf2, aes(x = inf2$first_appeared, y = inf2$lowest_price), ylim = 10) +
geom_line() +
coord_cartesian(ylim = c(0, 10))

inf3 <- inf1
inf3 <- inf3[inf3$lowest_price < mean(inf3$lowest_price) + 3*sd(inf3$lowest_price) & inf3$lowest_price > mean(inf3$lowest_price) - 3*sd(inf3$lowest_price),]
inf3 <- inf3[inf3$highest_price < mean(inf3$highest_price) + 3*sd(inf3$highest_price) & inf3$highest_price > mean(inf3$highest_price) - 3*sd(inf3$highest_price),]
inf3$first_appeared[inf3$first_appeared > 2023] <- NA
inf3$last_appeared[inf3$last_appeared > 2023] <- NA
inf3 <- na.omit(inf3)

summary(inf3)

ggplot(data = inf3, aes(x = inf3$first_appeared, y = inf3$lowest_price), ylim = 10) +
  geom_line() +
  coord_cartesian(ylim = c(0, 30))

ggplot(data = inf3, aes(x = inf3$last_appeared, y = inf3$highest_price), ylim = 10) +
  geom_line() +
  coord_cartesian(ylim = c(0, 50))

hist(inf3$lowest_price,xlim=c(0, 30), breaks = 1000, main="lowest_price")
hist(inf3$highest_price,xlim=c(0, 60), breaks = 2000, main="highest_price")
hist(inf3$first_appeared,xlim=c(1800, 2023), breaks = 100, main="first_appeared")
hist(inf3$last_appeared,xlim=c(1800, 2023), breaks = 100, main="last_appeared")

boxplot( inf3$lowest_price,
         boxwex = 0.5,
         medcol = "red")
boxplot( inf3$highest_price ,
         boxwex = 0.5,
         medcol = "red")


sampled_inf <- inf3[sample(nrow(inf3), 30), ]


dist.price <- dist(sampled_inf [,5])
clust.price <- hclust(dist.price, "ward.D")
plot(clust.price, labels = sampled_inf$name, main="Дендограмма",ylab="Сходство",xlab=",блюда")
